...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzle_ranges[i][j]= RangeProof(4);
            puzzle_ranges[i][j].range[0] <== 0;  // lower bound
            puzzle_ranges[i][j].range[1] <== 9;  // upper bound

            solution_ranges[i][j]= RangeProof(4);
            solution_ranges[i][j].range[0] <== 0;  // lower bound
            solution_ranges[i][j].range[1] <== 9;  // upper bound
        }
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            solution_ranges[i][j].in <== puzzle[i][j];
